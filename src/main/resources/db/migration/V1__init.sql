...
CREATE TABLE core_time_block (
    ...
    reason                   TEXT,
    -- Mirrors HyperBrain-Infra (ADR-027 D1): the themed container's name, generated by the LLM
    -- (ADR-029). Nullable: the deterministic floor may leave the theme derived.
    theme                    TEXT,
...
-- Mirrors HyperBrain-Infra/supabase/migrations/20260725120000_adr027_grouped_time_block_member.sql
-- (ADR-027 D2/D5): the block is a themed TIME CONTAINER referencing N executables. block_date and
-- block_status are DERIVED, trigger-owned copies of the parent block — the core never writes them;
-- they exist only so the D5 invariant ("<= 1 block per executable per day") can be enforced by a
-- real unique index (PostgreSQL cannot index across a join, and `AT TIME ZONE <tz>` is STABLE, not
-- IMMUTABLE, so a GENERATED column is impossible). The index is PARTIAL over live blocks: an
-- EXPIRED block is out of the plan, so a same-day replan may legitimately re-place its executable.
CREATE TABLE core_time_block_member (
    block_id        UUID    NOT NULL REFERENCES core_time_block (id) ON DELETE CASCADE,
    executable_id   UUID    NOT NULL REFERENCES core_executable (id) ON DELETE CASCADE,
...
CREATE OR REPLACE FUNCTION core_time_block_member_derive()
    RETURNS TRIGGER
    LANGUAGE plpgsql
AS $$
DECLARE
...
CREATE TRIGGER trg_core_time_block_member_derive
    ...
    ON core_time_block_member
    FOR EACH ROW
EXECUTE FUNCTION core_time_block_member_derive();

CREATE OR REPLACE FUNCTION core_time_block_propagate_to_members()
    RETURNS TRIGGER
    LANGUAGE plpgsql
AS $$
BEGIN
    UPDATE core_time_block_member m
...
EXECUTE FUNCTION core_time_block_propagate_to_members();

CREATE UNIQUE INDEX uq_core_time_block_member_executable_day
    ON core_time_block_member (executable_id, block_date)
