...
/**
 * DR-01: only the event-backed executables (ACTIVITY, AGENDA, LEARNING_SESSION and, since
 * ADR-039, TIME_BLOCK — the block IS an executable whose window needs both bounds) represent
 * fixed time blocks with a meaningful end boundary. For all other types (the reminder-backed
 * ones and any unsynced type) the end_time is cleared to prevent invalid date ranges reaching
 * downstream systems (e.g. Notion 400 when end &lt; start) and to keep a reminder's due date
 * at its start.
 */
@Component
public class EndTimeInvariantRule implements DomainRule {
    ...
    private static final Set<String> EVENT_TYPES =
        Set.of("ACTIVITY", "AGENDA", "LEARNING_SESSION", "TIME_BLOCK");
