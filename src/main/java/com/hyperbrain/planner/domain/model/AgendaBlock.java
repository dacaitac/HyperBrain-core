 ...
 *
 * <p><b>Theme.</b> The container's name is generated by the LLM (ADR-029) and lands on the block
 * executable's {@code name}. It is a presentation property <b>of the block</b> — it is never written
 * onto a member's canonical title (ADR-012 mirror hygiene). The deterministic floor leaves it null and
 * the persistence derives a readable default from the slot: under ADR-040 D5/D6 the floor lays and
 * orders, and naming is the LLM's work.
 *
 * <p><b>Template slot (ADR-040 D7/D14).</b> {@code templateSlotId} is the band of the day this block
 * realises, and it is what the block is <b>recognised by</b> across regenerations — never its name,
 * because if identity depended on the name the calendar event would duplicate every single time the
 * LLM did its job well. It is null for a block that comes from no template.
 *
 ...
 *                          normalized to null)
 * @param templateSlotId    the template band this block realises; null when it comes from no template
 */
public record AgendaBlock(
    ...
    public AgendaBlock {
        ...
        templateSlotId =
            templateSlotId == null || templateSlotId.isBlank() ? null : templateSlotId.strip();
    }

    /** Builds a single-executable, themeless, slot-less block. */
    ...
    /** Builds a themeless, slot-less block with an explicit membership. */
    public AgendaBlock(UUID executableId, OffsetDateTime start, OffsetDateTime end, boolean wig,
    ...
    /** Builds a slot-less block with an explicit membership and theme. */
    public AgendaBlock(UUID executableId, OffsetDateTime start, OffsetDateTime end, boolean wig,
                       boolean highLoad, String reason, List<UUID> additionalMembers, String theme) {
        this(executableId, start, end, wig, highLoad, reason, additionalMembers, theme, null);
    }
    ...
    public AgendaBlock retimed(OffsetDateTime newStart, OffsetDateTime newEnd, String newReason) {
        return new AgendaBlock(executableId, newStart, newEnd, wig, highLoad, newReason,
            additionalMembers, theme, templateSlotId);
    }
