 ...
 * imports the Tasks and Cycles databases (inbound). Since ADR-039 the {@code TIME_BLOCK}
 * executables live in the Tasks database, so the single Tasks pass covers them — there is no
 * separate Time Blocks endpoint any more. Internal operations endpoint — the Core is never
 * exposed publicly ({@code daniel-ubuntu} stays private, ADR-011); same trust model as the
 * SentinelAPI {@code /resync}. Thin controller: delegates and serializes the summary.
 */
@RestController
